package es.asanchez.kafka.consumer.config;
...
public class KafkaConsumerConfig<K extends Serializable, V extends SpecificRecordBase> {
    ...
    public KafkaListenerContainerFactory<ConcurrentMessageListenerContainer<K, V>> kafkaListenerContainerFactory() {
        ...
        factory.setBatchListener(kafkaConsumerConfigData.getBatchListener()); //No queremos los datos 1 por uno si no por lotes
        factory.setConcurrency(kafkaConsumerConfigData.getConcurrencyLevel()); //Hilos que creara spring para consumir los datos (Igual al numero de particiones)
        factory.setAutoStartup(kafkaConsumerConfigData.getAutoStartup()); //Inicializar automaticamente cuando la aplicacion se inicie
        factory.getContainerProperties().setPollTimeout(kafkaConsumerConfigData.getPollTimeoutMs()); //Tiempo de espera antes del ultimo "record" cuando llamamos al poll
        return factory;
    }
